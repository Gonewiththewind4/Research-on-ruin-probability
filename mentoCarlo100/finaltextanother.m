function [aaaa,bbbb,cccc]=finaltextanother(ppp,n)
%clear all
u=2.5;%均值
 o=1;%方差
 n=n;%个数
...
%while (E<n)
 ...
 %while((abs(tol1)>0.6*u)&&(abs(tol2)>0.6*o))
...
g1=varSt-(1/(O.^2))*((2*exp(yi+er)+2.*n)+(exp(2*yi+4*er)-exp(2*yi+2*er)));
f1=diff(varSt-(1/(O.^2))*((2*exp(yi+er)+n)+(exp(2*yi+4*er)-exp(2*yi+2*er))));
...
syms u3 g2 f2;
g2=varSt-(1/(O.^2))*((2*exp(u3/(1-a11(acb))+(o1(acb).^2)/(2*(1-a11(acb).^2)))+n)+(exp(2*u3/(1-a11(acb))+4*(o1(acb).^2)/(2*(1-a11(acb).^2)))-exp(2*u3/(1-a11(acb))+2*(o1(acb).^2)/(2*(1-a11(acb).^2)))));
f2=diff(varSt-(1/(O.^2))*((2*exp(u3/(1-a11(acb))+(o1(acb).^2)/(2*(1-a11(acb).^2)))+n)+(exp(2*u3/(1-a11(acb))+4*(o1(acb).^2)/(2*(1-a11(acb).^2)))-exp(2*u3/(1-a11(acb))+2*(o1(acb).^2)/(2*(1-a11(acb).^2))))));
u3=1;
Tol=1e-6;
a2=2;
%er1=eval(er);
5;
while (abs(a2-u3)>Tol)%((num2str(a1)-num2str(er))>num2str(Tol)) | ((num2str(er)-num2str(a1))>num2str(Tol))
    u3=a2;
    u2=g2/f2;
    k1=subs(u2,'u3',[u3]);
    k1=eval(k1);
    a2=u3-k1;%num2str(g)/num2str(f);
end
6;
er4=a2;
Uguji(acb)=er4;
